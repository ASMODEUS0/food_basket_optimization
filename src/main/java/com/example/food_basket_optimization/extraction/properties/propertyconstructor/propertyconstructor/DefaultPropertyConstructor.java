...
public class DefaultPropertyConstructor<T extends SimpleProperty<?>> extends PropertyConstructorAbs<T> {


    public DefaultPropertyConstructor(PropertySource source, Constructor<T> constructor) {
    ...
    public List<T> postConstruct(List<ExtractedEntity> refEntities) {
        ...
        return listOfParams.stream().map(params-> Util.createObjectFromMultipliedParams(constructor, params)).toList();
    }
