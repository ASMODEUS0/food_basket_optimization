...
@Component
public class SourceHttpResolver implements SourceResolverContract<ResolvableSource<SourceHttpContract>> {
    ...
    @Override
    public List<MapProperty> getData(ResolvableSource<SourceHttpContract> resolvableSource, Class<? extends ExtractedEntity> classToParse) {
        List<SourceHttpContract> resolvedSources = resolvableSource.resolve();
        ...
        for (SourceHttpContract source : resolvedSources) {
            ...
            Stream.of(source).map(this::createRequestFrom).map(request -> {
                    ...
                    }).map(data -> new HtmlMapProperty(data, classToParse, source.getRefProperties()))
                    .findFirst()
    ...
    private HttpRequest createRequestFrom(SourceHttpContract source) {
        URI uri = URI.create(source.getUrl().toString());
        ...
        return RequestUtill.resolveHttpMethod(requestBuilderWithHeaders, source.getMethod(), source.getBody()).build();
    }
